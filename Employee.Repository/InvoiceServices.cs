...
namespace Employee.Repository
{
    public class InvoiceServices : IInvoiceRepository
    {
        ...
        public async Task<Response> Delete(int InvoiceID)
        {
            try
            {
                using (var connection = new SqlConnection(_connectionString))
                {
                    DynamicParameters para = new DynamicParameters();
                    para.Add("@InID", InvoiceID);
                    var results = await connection.QueryAsync<EmployeeClass>("[dbo].[DeleteInvoice]", para, commandType: CommandType.StoredProcedure);
